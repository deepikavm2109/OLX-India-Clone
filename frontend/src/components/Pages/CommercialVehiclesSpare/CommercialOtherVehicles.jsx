...
const CommercialOtherVehicles = () => {
  ...
  console.log("--cat--", category);      // Commercial Vehicles & Spares
  console.log("--subcat--", subcategory); // Commercial & Other Vehicles
  ...
  const validateForm = () => {
    ...
    if (!formData.cvsYear) {
      newErrors.cvsYear = "Year is mandatory";
    } else if (!/^\d{4}$/.test(formData.cvsYear)) {
      newErrors.cvsYear = "Year must be a 4-digit number";
    } else if (formData.cvsYear < 1980) {
      newErrors.cvsYear = "Model is too old to be accepted";
    } else if (formData.cvsYear > new Date().getFullYear()) {
      newErrors.cvsYear = "Year has not yet occurred";
    }
    if (!formData.cvsKMDriven) {
      newErrors.cvsKMDriven = "KM is mandatory";
    }
  ...
  const cvsType = ["Auto-rickshaws & E-rickshaws", "Buses", "Trucks","Heavy Machinery","Modified Jeeps", "Pick-up vans / Pick-up trucks","Scrap cvss","Taxi Cabs","Tractors","Others"];
  ...
  const handleSubmit = async(e) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      uploadData.append("cvsYear", formData.cvsYear);
      uploadData.append("cvsKMDriven", formData.cvsKMDriven);
    ...
    <>
        ...
        {JSON.stringify(
          {
            ...
            cvsYear: formData.cvsYear,
            cvsKMDriven: formData.cvsKMDriven,
      ...
      <form className="form-title" onSubmit={handleSubmit}>
        ...
        <div className="form-div">
          ...
          <Form.Control as="select" className="form-field" onChange={(e) =>
              setFormData({ ...formData, cvsType: e.target.value })
            }>
              <option value=""></option>
        ...
        <div className="form-div">
          ...
          <Form.Control
            ...
            value={formData.cvsYear}
            onChange={(e) =>
              setFormData({ ...formData, cvsYear: e.target.value })
            }
          />
          {errors.cvsYear && (
            <p className="error-message">{errors.cvsYear}</p>
          )}
        </div>
        ...
        <div className="form-div">
          ...
          <Form.Control
            ...
            value={formData.cvsKMDriven}
            onChange={(e) =>
              setFormData({ ...formData, cvsKMDriven: e.target.value })
            }
          />
          {errors.cvsKMDriven && (
            <p className="error-message">{errors.cvsKMDriven}</p>
          )}
        </div>
...
export default CommercialOtherVehicles;
