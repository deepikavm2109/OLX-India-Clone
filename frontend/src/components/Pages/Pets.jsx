...
const Pets = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.petTitle) {
      newErrors.petTitle = "Ad Title is mandatory";
    }
    if (!formData.petDescription) {
      newErrors.petDescription = "Description is mandatory";
    }

    if (!formData.petPrice) {
      newErrors.petPrice = "Price is mandatory";
    } else if (isNaN(formData.petPrice) || formData.petPrice <= 0) {
      newErrors.petPrice = "Price must be a positive number";
    }
    if (!formData.petState) {
      newErrors.petState = "State is mandatory";
    }

    if (formData.petPhotos.length === 0) {
      newErrors.petPhotos = "Atleast one image is mandatory";
    }
  ...
  const handleSubmit = async(e) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      uploadData.append("petTitle", formData.petTitle);
      uploadData.append("petDescription", formData.petDescription);
      uploadData.append("petPrice", formData.petPrice);
      uploadData.append("petState", formData.petState);
      uploadData.append("username", formData.username);
      uploadData.append("email", formData.email);
      // uploadData.append("petPhotos",formData.petPhotos);
      // for single photos

      formData.petPhotos.forEach((image, index) => {
        uploadData.append(`petPhotos[${index}]`, image);
      });
      ...
      try {
        ...
        const response = await axios.post(
          "http://127.0.0.1:8000/olxapi/petform/",
          uploadData,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
        ...
        console.log("pet Data successfully posted:", response.data);
        ...
        {JSON.stringify(
          {
            ...
            petTitle: formData.petTitle,
            petDescription: formData.petDescription,
            petPrice: formData.petPrice,
            petState: formData.petState,
            petPhotos: formData.petPhotos,
      ...
      <form className="form-title" onSubmit={handleSubmit}>
        ...
        <div className="form-div">
          ...
          <Form.Control
            ...
            value={formData.petTitle}
            onChange={(e) =>
              setFormData({ ...formData, petTitle: e.target.value })
            }
          />
          ...
          {errors.petTitle && (
            <p className="error-message">{errors.petTitle}</p>
          )}
        </div>
        ...
        <div className="form-div">
          ...
          <Form.Control
            ...
            onChange={(e) =>
              setFormData({ ...formData, petDescription: e.target.value })
            }
          />
          <span className="form-description">
          ...
          {errors.petDescription && (
            <p className="error-message">{errors.petDescription}</p>
          )}
        </div>
        ...
        <div className="form-div">
          ...
          <Form.Control
            ...
            onChange={(e) =>
              setFormData({ ...formData, petPrice: e.target.value })
            }
            style={{
          ...
          />
          {errors.petPrice && (
            <p className="error-message">{errors.petPrice}</p>
          )}
        </div>
        <hr />
        ...
        <div className="form-div">
          ...
          <Form.Group controlId="formpetPhotos">
            <Form.Label
              style={{
                border: "1px solid",
                width: "150px",
          ...
          {errors.petPhotos && (
            <p className="error-message">{errors.petPhotos}</p>
          )}
        </div>
        ...
        <div className="form-div">
          ...
          <Form.Control
            ...
            className="form-field"
            onChange={(e) =>
              setFormData({ ...formData, petState: e.target.value })
            }
          >
        ...
        {errors.petState && <p className="error-message">{errors.petState}</p>}
        <hr />
