...
const MobilePhones = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.mobileBrand) {
      newErrors.mobileBrand = "Brand is mandatory";
    }

    if (!formData.mobileTitle) {
      newErrors.mobileTitle = "Ad Title is mandatory";
    }
    if (!formData.mobileDescription) {
      newErrors.mobileDescription = "Description is mandatory";
    }

    if (!formData.mobilePrice) {
      newErrors.mobilePrice = "Price is mandatory";
    } else if (isNaN(formData.mobilePrice) || formData.mobilePrice <= 0) {
      newErrors.mobilePrice = "Price must be a positive number";
    }
    if (!formData.mobileState) {
      newErrors.mobileState = "State is mandatory";
    }

    if (formData.mobilePhotos.length === 0) {
      newErrors.mobilePhotos = "Atleast one image is mandatory";
    }
  ...
  const fetchBrandTitle = async () => {
    try {
      const titleResponse = await axios.get(`/olxapi/mobilebrandtitle/`);
      ...
      for (const brand of titleResponse.data) {
        ...
        const subTitleResponse = await axios.get(
          `/olxapi/mobilebrandsubtitle/?brand_title_id=${brand.id}`
        );
  ...
  const handleSubmit = async(e) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      // uploadData.append("mobileSubcategory", formData.mobileSubcategory);
      uploadData.append("mobileBrand", formData.mobileBrand);
      uploadData.append("mobileTitle", formData.mobileTitle);
      uploadData.append("mobileDescription", formData.mobileDescription);
      uploadData.append("mobilePrice", formData.mobilePrice);
      uploadData.append("mobileState", formData.mobileState);
      uploadData.append("username", formData.username);
      uploadData.append("email", formData.email);
      // uploadData.append("mobilePhotos",formData.mobilePhotos);
      // for single photos

      formData.mobilePhotos.forEach((image, index) => {
        uploadData.append(`mobilePhotos[${index}]`, image);
      });
      ...
      try {
        ...
        const response = await axios.post(
          "http://127.0.0.1:8000/olxapi/mobileform/",
          uploadData,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
        ...
        console.log("mobile Data successfully posted:", response.data);
    ...
    <>
        ...
        {JSON.stringify(
          {
            ...
            mobileBrand: formData.mobileBrand,
            mobileTitle: formData.mobileTitle,
            mobileDescription: formData.mobileDescription,
            mobilePrice: formData.mobilePrice,
            mobileState: formData.mobileState,
            mobilePhotos: formData.mobilePhotos,
      ...
      <form className="form-title" onSubmit={handleSubmit}>
        ...
        <div className="form-div">
            ...
            {brandTitle.map((title) => (
              <optgroup label={title.name} key={title.id}>
                {brandSubtitle[title.id]?.map((brandsubtitle) => (
                  ...
                  <option value={brandsubtitle.name} key={brandsubtitle.id}>
                    {brandsubtitle.name}
                  </option>
                ))}
              </optgroup>
          ...
          {errors.mobileBrand && (
            <p className="error-message">{errors.mobileBrand}</p>
          )}
        </div>
        ...
        <div className="form-div">
          ...
          <Form.Control
            ...
            onChange={(e) =>
              setFormData({ ...formData, mobileTitle: e.target.value })
            }
          />
          ...
          {errors.mobileTitle && (
            <p className="error-message">{errors.mobileTitle}</p>
          )}
        </div>
        ...
        <div className="form-div">
          ...
          <Form.Control
            ...
            onChange={(e) =>
              setFormData({ ...formData, mobileDescription: e.target.value })
            }
          />
          <span className="form-description">
          ...
          {errors.mobileDescription && (
            <p className="error-message">{errors.mobileDescription}</p>
          )}
        </div>
        ...
        <div className="form-div">
          ...
          <Form.Control
            ...
            onChange={(e) =>
              setFormData({ ...formData, mobilePrice: e.target.value })
            }
            style={{
          ...
          />
          {errors.mobilePrice && (
            <p className="error-message">{errors.mobilePrice}</p>
          )}
        </div>
        <hr />
        ...
        <div className="form-div">
          ...
          <Form.Group controlId="formmobilePhotos">
            <Form.Label
              style={{
                border: "1px solid",
                width: "150px",
          ...
          {errors.mobilePhotos && (
            <p className="error-message">{errors.mobilePhotos}</p>
          )}
        </div>
        ...
        <div className="form-div">
          ...
          <Form.Control
            ...
            className="form-field"
            onChange={(e) =>
              setFormData({ ...formData, mobileState: e.target.value })
            }
          >
        ...
        {errors.mobileState && <p className="error-message">{errors.mobileState}</p>}
        <hr />
